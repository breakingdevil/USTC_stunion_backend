...
def caslogin():
    ...
    if ticket:
        ...
        if cas_response and cas_response.success:
            ...
            myrecord = User.query.filter_by(school_id=cas_response.user).first()
            if myrecord is None:
                newuser = User(school_id=cas_response.user)
                db.session.add(newuser)
                db.session.commit()
                newuser = User.query.filter_by(school_id=cas_response.user).first()
                login_user(newuser)
